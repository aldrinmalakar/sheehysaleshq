/* ============================================================
   SHEEHY SALES HQ - shared navigation, profile and page loaders
============================================================ */
(function(){
'use strict';
...
'.shq-inner{max-width:1120px;margin:0 auto;display:flex;align-items:center;gap:10px;padding:8px 14px;flex-wrap:wrap}',
'.shq-brand{text-decoration:none;color:#1a2330;font-size:16px;font-weight:650;white-space:nowrap}',
...
'.shq-clock-status{font-size:11px;font-weight:850;color:#536174}',
...
'.shq-links a{text-decoration:none;font-size:12px;font-weight:650;color:#6b7889;border:1px solid #e4e8ee;border-radius:8px;padding:6px 10px;background:#fff;white-space:nowrap}',
...
function syncProfile(){
  var p=loadProfile();
  document.querySelectorAll('[data-f]').forEach(function(el){var k=el.getAttribute('data-f');if(!k)return;if((!el.value||!String(el.value).trim())&&p[k]){el.value=p[k];try{el.dispatchEvent(new Event('change',{bubbles:true}));}catch(e){}}el.addEventListener('input',function(){var q=loadProfile();q[k]=el.value||'';saveProfile(q);});el.addEventListener('change',function(){var q=loadProfile();q[k]=el.value||'';saveProfile(q);});});
}
function normalizeInternalLinks(){document.querySelectorAll('a[href]').forEach(function(a){var h=a.getAttribute('href')||'';if(!h||h.indexOf('javascript:')===0)return;try{var u=new URL(h,location.href);if(u.origin===location.origin){a.removeAttribute('target');a.removeAttribute('rel');}}catch(e){}});}

var SUBJECT_PAGES={'reconnect.html':true};
...
function emojiFor(s){var x=stripEmoji(s).toLowerCase();if(/video|show you/.test(x))return '🎥';if(/appointment|you are set|set for|today or tomorrow/.test(x))return '📅';if(/review/.test(x))return '⭐';if(/inventory|back in stock/.test(x))return '🚙';if(/still|missed|picking .* back up/.test(x))return '👋';return '';}
...
function initSubjectStyle(){if(!SUBJECT_PAGES[here])return;function apply(){var r=document.getElementById('subjOut');if(r&&r.value)r.value=formatSubject(r.value);}apply();['change','click'].forEach(function(ev){document.addEventListener(ev,function(){setTimeout(apply,0);});});}

var WORDTRACK_PAGES={'reconnect.html':true};
var VOICE_PAGES={'objection-library.html':true,'reconnect.html':true,'after-sale.html':true,'survey.html':true,'sister-store.html':true};
function initSalesVoice(){if(VOICE_PAGES[here])loadScriptOnce('shqSalesVoiceScript','./sales-voice.js');}
function initCanonicalWordtracks(){if(!WORDTRACK_PAGES[here])return initSalesVoice();function finish(){initSalesVoice();}function install(){if(window.SHQWordtracks){window.SHQWordtracks.install(here);finish();return;}loadScriptOnce('shqWordtracksScript','./wordtracks.js',function(){if(window.SHQWordtracks)window.SHQWordtracks.install(here);finish();});}function warm(){loadScriptOnce('shqFunnelWarmConfidenceScript','./funnel-warm-confidence.js',install);}function conf(){loadScriptOnce('shqFunnelConfidenceScript','./funnel-confidence.js',warm);}if(window.SHQFunnel)conf();else loadScriptOnce('shqFunnelDataScript','./funnel-data.js',conf);}
function initPageEnhancements(){
  loadScriptOnce('shqEmailSignatureCleaner','./email-signature-cleaner.js');
  if(here==='funnel.html')loadScriptOnce('shqFunnelProVoicemail','./funnel-pro-voicemail.js');
  if(here==='reconnect.html')loadScriptOnce('shqReconnectLiveCoach','./reconnect-live-coach.js');
  if(here==='otd-calculator.html')loadScriptOnce('shqOtdTermExtension','./otd-term-extension.js');
  if(here==='sister-store.html')loadScriptOnce('shqSharedInventoryUI','./shared-inventory-ui.js');
  if(here==='survey.html'||here==='after-sale.html')loadScriptOnce('shqVocSalesSurvey','./voc-sales-survey.js');
}
function init(){build();syncProfile();normalizeInternalLinks();initSubjectStyle();initCanonicalWordtracks();initPageEnhancements();if(window.MutationObserver&&document.body)new MutationObserver(function(){normalizeInternalLinks();}).observe(document.body,{childList:true,subtree:true});}
