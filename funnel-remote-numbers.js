...
function tok(name){var el=document.querySelector('[data-f="'+name+'"]'),fallback={name:'[Name]',vehicle:'[vehicle]',current:'[current]',daytime:'[day/time]',alttime:'[alt time]',agent:'[agent]',number:'[number]',email:'[email]'};return el&&el.value&&el.value.trim()?el.value.trim():(fallback[name]||'['+name+']');}
