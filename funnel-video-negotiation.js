...
function plan(){var p=ptext(),n=name(),v=vehicle(),a=agent(),remote=longDistance();return [
  ...
  ['Surface the risk instead of hiding it','If there is a condition concern, difference, missing feature or likely objection, show it clearly now. A surprise later kills more deals than a truthful detail now.'],
...
function companion(){var p=ptext();return 'Hi '+name()+',\n\nI made this around '+p+' instead of doing a generic walk-around. Watch that part first.\n\nIf it checks the box, tell me what, if anything, is still missing and I’ll work that next.\n\n'+agent();}
function apply(){var six=$('sixpo'),box=$('videoCompanion'),panel=$('videoPanel');if(!six||!box||!panel||panel.hidden||stage()==='outbound'||behavior()==='unit-gone')return;if(six.querySelector('[data-negotiation-video="1"]'))return;var html='',pts=plan();pts.forEach(function(p,i){html+=point(i+1,p[0],p[1]);});six.innerHTML=html;box.innerHTML='<h3>Send with the video</h3><div class="notice" style="margin-top:0;white-space:pre-wrap">'+esc(companion())+'</div><div class="quiet" style="margin-top:8px">Video rule: label the customer’s priority, prove it, surface the likely concern, then ask what is still missing. Do not keep presenting after you have earned the decision question.</div>';}
