...
function programName(){var p=profile();return p.program&&String(p.program).trim()?String(p.program).trim():'the program you received';}
...
patch('owner-wants-value',{
  ...
  email:`Hi [Name],\n\nI can help get a real value on your [current]. I need the basics before I ask anyone to put a number on it.\n\nSend me the approximate mileage and anything material about condition. I will line up the right appraisal step from there.\n\n[agent]`,
  video:`Hi [Name], [agent] at Sheehy Nissan. On your [current], give me the approximate miles and condition and I will line up the real appraisal path.`
});
...
add('owner-why-calling','Asks, “Why Are You Calling Me?”','The owner challenges the reason for the outreach.','Answer in one sentence, remove mystery and earn the next question.','Verify whether a vehicle change is even relevant.',
`Fair question. I am calling because we have you associated with a [current], and I am trying to find out whether it is still doing the job or whether a change is worth discussing. Are you planning to keep it, or would the right reason make you consider changing it?`,
`Hi [Name], [agent] at Sheehy Nissan. I am calling about your [current]. I want to know whether you are keeping it or whether a change is worth a conversation. Call or text me at [number].`,
...
`Hi [Name], [agent] at Sheehy Nissan. I want to know whether your [current] is still the right vehicle for you or whether a change is worth discussing.`);
...
`Hi [Name],\n\nGood to know you still have the [current]. What is one thing you still like about it and one thing you would change if you could?\n\nThat tells me whether there is anything worth working on.\n\n[agent]`,
`Hi [Name], [agent] at Sheehy Nissan. Tell me one thing you still like about your [current] and one thing you wish were better.`);
...
`Good. If the [current] is doing the job, keeping it may be the right move. One thing before I let you go: is that because you genuinely want to keep it, or because the timing and numbers do not make sense right now?`,
...
`If the [current] is doing the job, keeping it may be the right move. Quick distinction: do you genuinely want to keep it, or is it mainly that the timing/numbers do not make sense right now?`,
`[Name], One Last Question on the [current]`,
`Hi [Name],\n\nIf the [current] is doing the job, keeping it may be the right move.\n\nDo you genuinely want to keep the vehicle, or is the real issue that the timing or numbers do not make sense right now?\n\n[agent]`,
`No walkaround needed. Find out whether “keeping it” means satisfied, bad timing or bad numbers.`);

add('owner-later','Not Now / Maybe Later','The owner is not rejecting a future change but says the timing is later.','Find the real future trigger instead of creating endless follow-up.','Identify what has to change and establish a useful follow-up point.',
`Understood. When you say later, what are you actually waiting on: better timing, the numbers making sense, a specific vehicle or simply a real reason to change the [current]?`,
`Hi [Name], [agent] at Sheehy Nissan. I heard you on the timing being later. I have one quick question so I follow up for a reason instead of guessing. Call or text me at [number].`,
`When you say later, what are you waiting on: timing, numbers, a specific vehicle or simply a real reason to change the [current]?`,
`[Name], What Would Make the Timing Right?`,
`Hi [Name],\n\nI heard you on the timing being later. What are you actually waiting on: better timing, the numbers making sense, a specific vehicle or simply a real reason to change the [current]?\n\nOnce I know that, I can follow up around something real.\n\n[agent]`,
`No generic walkaround. If they name a specific future trigger, build the follow-up around that trigger.`);
...
`Perfect. What would have to improve over your [current] for a change to be worth it: space, comfort, technology, reliability, ownership cost or something else?`,
...
`Hi [Name],\n\nIf you are open to changing the [current], let us get specific. What has to improve for the move to be worth it: space, comfort, technology, reliability, ownership cost or something else?\n\nGive me the top one and I will narrow the direction.\n\n[agent]`,
`Hi [Name], [agent] at Sheehy Nissan. If you are open to changing the [current], tell me what actually needs to be better. I will narrow the direction from there.`);
...
`Hi [Name],\n\nSince you already like the [current], the comparison is simple. What is one thing you refuse to lose from yours, and what is one thing you want the newer vehicle to improve?\n\nI will use that to line up the closest current match.\n\n[agent]`,
...
`That gives us a real reason to look. What changed in your life or use of the vehicle that the [current] is not handling well anymore?`,
...
`Hi [Name],\n\nIf the [current] no longer fits, what changed: space, commute, comfort, family use, towing, fuel cost or something else?\n\nOnce I know the actual problem, I can narrow the right vehicle.\n\n[agent]`,
`Make the video about the need they named. If they need space, prove space. If they need comfort, prove seating and ride-related features.`);
...
add('owner-program-question','Asks About the Program / Letter / Email','The owner asks what the campaign, letter, email or program means.','Answer the question, then determine whether there is actual replacement intent.','Name the selected program when available, keep eligibility unpromised and ask whether they are actually considering a change.',
`Absolutely. The one you are asking about is [program]. I can explain it, but I do not want to tell you it applies until I verify the current rules. Are you considering replacing the [current], or are you mainly trying to understand the program?`,
`Hi [Name], [agent] at Sheehy Nissan. I can explain [program] and verify the current rules before we count anything into a deal. Call or text me at [number].`,
`The one you are asking about is [program]. I can verify the current rules before we count anything into a deal. Are you considering replacing the [current], or mainly trying to understand the program?`,
`[Name], About [program]`,
`Hi [Name],\n\nThe one you are asking about is [program]. I can explain it, but I do not want to imply eligibility or savings before I verify the current rules.\n\nAre you considering replacing the [current], or are you mainly trying to understand the program?\n\n[agent]`,
...
add('owner-sell-only','Wants to Sell, Not Replace','The owner says they may sell the current vehicle but are not shopping for a replacement.','Keep the conversation on the vehicle and route it to the proper used-car appraisal path.','Confirm the basic vehicle facts and get the used-car side to determine the next appraisal step.',
`Understood. If you are looking to sell the [current] without replacing it, give me the approximate miles and condition and I will confirm the right appraisal path from there.`,
...
`Hi [Name],\n\nIf you are looking to sell the [current] without replacing it, send me the approximate mileage and anything material about condition. I will confirm the right appraisal path from there.\n\n[agent]`,
`No replacement-vehicle video. If useful, make a brief face-to-name message asking for mileage and material condition only.`);

add('owner-already-replaced','Already Replaced / Bought Something Else','The owner says they already replaced the vehicle or recently bought something else.','Stop the sales pursuit and preserve the relationship.','Congratulate them, learn what they bought only if natural and close cleanly.',
